...
Stmt:	LVal ASSIGN Exp SEMICOLON {
		...
		|IF LP Cond RP Stmt {
			$$ = new StmtAST();
			$$->sType = SEL;						//if is select stmt
			auto select = new SelectStmtAST();
			select->cond = unique_ptr<LOrExpAST>($3);
			select->ifStmt = unique_ptr<StmtAST>($5);
			$$->selectStmt = unique_ptr<SelectStmtAST>(select);
		}
		|IF LP Cond RP Stmt ELSE Stmt {
			...
			select->cond = unique_ptr<LOrExpAST>($3);
			select->ifStmt = unique_ptr<StmtAST>($5);
			select->elseStmt = unique_ptr<StmtAST>($7);
			$$->selectStmt = unique_ptr<SelectStmtAST>(select);
		}
		|WHILE LP Cond RP Stmt {
			$$ = new StmtAST();
			$$->sType = ITER;						//while is iterator stmt
			auto iteration = new IterationStmtAST();
			iteration->cond = unique_ptr<LOrExpAST>($3);
			iteration->stmt = unique_ptr<StmtAST>($5);
			$$->iterationStmt = unique_ptr<IterationStmtAST>(iteration);
		}
		|BREAK SEMICOLON {
			$$ = new StmtAST();
			$$->sType = BRE;
